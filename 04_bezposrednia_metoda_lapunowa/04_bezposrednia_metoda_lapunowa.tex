...
\title{
    Teoria Sterowania\\
    \Large Bezpośrednia Metoda Lapunowa
}
...
W niniejszym materiale kontynuujemy numerację twierdzeń i przykładów ze wstępu do pośredniej metody Lapunowa, gdyż treści te się uzupełniają i koncepcyjnie tworzą spójną całość.
Kilka definicji wymaganych do wprowadzenia metody bezpośredniej zostało skopiowanych ze wstępu do metody pośredniej. Niemniej zaleca się,
...
Twierdzenie Lapunowa dostarcza warunek wystarczający stabilności punktu równowagi, jednak nie dostarcza żadnej informacji o tym, jak odnaleźć funkcję $V(x)$. Odnalezienie tej funkcji jest zasadniczą trudnością w badaniu stabilności.
W punkcie \ref{sec:dobor_funkcji} podano kilka metod poszukiwania funkcji Lapunowa.
\end{tcolorbox}

\textbf{\textit{Dowód:}}
\textit{Stabilność:}
Zgodnie z \textbf{\textit{Definicją 4}} weźmy liczbę $\epsilon > 0$.
...
Wzdłuż tej trajektorii mamy $\dot{V}(x) \leq 0$, więc $V(x(t)) \leq V(x_{0}) \leq \beta$, czyli cała trajektoria $x(t)$ pozostanie w zbiorze $\Omega_{\beta} \subset K_{\epsilon}$ dla dowolnego $t$.
...
$$
jest prawdziwa i zgodnie z \textbf{\textit{Definicją 4}} $x_{e} = 0$ jest stabilny w sensie Lapunowa.
...
\textbf{Definicja 13}
Funkcja $V:D\rightarrow\mathbb{R}$ spełniająca założenia twierdzenia Lapunowa jest nazywana \textbf{funkcją Lapunowa}.
...
Jeżeli istnieje funkcja $V:D\rightarrow \mathbb{R}$ różniczkowalna w sposób ciągły, dodatnio określona w pewnym otoczeniu punktu równowagi $x_{e} = 0$ i taka, że jej pochodna systemowa $\dot{V}:D\rightarrow\mathbb{R}$ jest dodatnio określona w pewnym zbiorze $G = \left\{ x \in K_{\epsilon} : V(x) > 0 \right\}$, gdzie $K_{\epsilon} = \left\{ x \in \mathbb{R}^{n}: \lVert x \rVert \leq \epsilon \right\} \subset D$, to punkt równowagi $x_{e} = 0$ jest \textbf{niestabilny}.
...
jeśli istnieje radialnie nieograniczona, ciągle różniczkowalna, dodatnio określona w $\mathbb{R}^{n}$ funkcja $V: D \rightarrow \mathbb{R}$, taka, że jej pochodna systemowa jest ujemnie określona w $\mathbb{R}^{n}$, to punkt równowagi $x_{e} = 0$ układu (\ref{eq:sys_stac}) jest \textbf{globalnie asymptotycznie stabilny (GAS)}.
...
\begin{tcolorbox}[colback=green!5, colframe=green!75!black, title=Przykład]
    ...
    $$
        ...
        \begin{array}{l}
             \dot{x}_{1} = x_{1} + x_{2}\\
             \dot{x}_{2} = -\frac{x_{1}}{(1 + x_{1}^{2})^{2}} - x_{2}
        \end{array}
        \right\},
    $$
    posiadający jedyny punkt równowagi w początku układu współrzędnych,
    dla którego kandydatem na funkcję Lapunowa jest:
    $$
        V(x) = \frac{x_{1}^{2}}{1 + x_{1}^{2}} + x_{2}^{2}.
    $$
    $V(x)$ jest dodatnio określona dla wszystkich $x \neq 0$, a jej pochodna systemowa ma postać:
    $$
        \dot{V}(x) = -\frac{2x_{1}^{2}}{(1 + x_{1}^{2})^{2}} - 2x_{2}^{2}
    $$
    jest ujemnie określona wszędzie poza punktem równowagi systemu.

    Sprawdźmy granice. Jeśli $x_2=0$ i $x_1 \rightarrow \infty$:
    $$
        \lim_{x_1 \rightarrow \infty} V(x_1, 0) = \lim_{x_1 \rightarrow \infty}  \frac{x_{1}^{2}}{1 + x_{1}^{2}} = 1.
    $$
    Zatem funkcja Lapunowa nie jest radialnie nieograniczona, a tym samym nie możemy
    wnioskować o globalnej stabilności asymptotycznej punktu równowagi.
    Poziomice $V(x)=c$ dla $c<1$ są krzywymi zamkniętymi. Jednak dla $c=1$,
    poziomica to $x_2^2 = 1 - \frac{x_1^2}{1 + x_1^2} = \frac{1}{1 + x_1^2}$,
    czyli $x_2 = \pm \frac{1}{\sqrt{1 + x_1^2}}$. Ta krzywa rozciąga się do
    nieskończoności wzdłuż osi $x_1$.
    
    Jeśli warunek radialnej nieograniczoności nie jest spełniony, nie możemy
    zagwarantować, że trajektorie startujące z dalekich punktów (gdzie $V(x)\geq
    1$) zostaną „ściągnięte” do początku układu. Mogą one uciec do
    nieskończoności, mimo że ich „energia” $V$ stale maleje (dąży np. do wartości
    $1$ od góry).
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{fig/radialna_nieograniczonosc.eps}
        \caption{Funkcja Lapunowa $V(x)$ jest radialnie ograniczona -
        zbiór poziomicowy dla $V(x) = 1$ jest niedomknięty.}
    \end{figure}
\end{tcolorbox}

\begin{tcolorbox}[colback=blue!5, colframe=blue!75!black, title=Ważne]
Aby udowodnić Globalną Stabilność Asymptotyczną (GAS), „miska” potencjału $V$
musi rosnąć w nieskończoność w każdym kierunku. Jeśli miska ma „krawędź” na
skończonej wysokości (tak jak funkcja $\frac{x_1^2}{1 + x_1^2}$), trajektoria
może się z niej „wylać”, a udowodniona zostanie co najwyżej stabilność lokalna
wewnątrz tej "miski".
\end{tcolorbox}
...
\begin{equation}\label{eq:stab_wyk}
    ...
    \end{array}
\end{equation}
to punkt równowagi $x_{e} = 0$ układu (\ref{eq:sys_stac}) jest \textbf{wykładniczo stabilny}. Jeżeli $D = \mathbb{R}^{n}$, to punkt równowagi jest globalnie wykładniczo stabilny.

\textbf{Definicja 15}
Zbiór $M$ w przestrzenie stanów układu (\ref{eq:sys_stac}) jest nazywany \textbf{niezmienniczym (inwariantnym)}, jeśli każda trajektoria rozpoczynająca się w $M$ w nim pozostaje, czyli:
...
\textbf{Definicja 16}
Zbiór $N$ w przestrzeni stanów nazywamy \textbf{zbiorem granicznym} dla trajektorii $x(t)$ układu (\ref{eq:sys_stac}), jeśli dla dowolnego punktu $p \in N$ istnieje ciąg $x(t_{n}) \rightarrow p$, $t_{1} < t_{2} < \cdots$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{fig/invariant_set.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\linewidth]{fig/la_salle_przyklad.PNG}
    \caption{Ilustracja do twierdzenia o obszarze przyciągania.}
    \label{fig:la_calle_przyklad}
\end{figure}
Dość łatwo zauważyć, że obszar pomiędzy $-1$ a $1$ jest stabilny - dla dodatnich
położeń prędkość jest ujemna i ściąga stan do zera, dla ujemnych jest dodatnia i
też przyciąga stan do zera.
Natomiast poza tym obszarem stan oddala się do nieskończoności - położenia
powyżej/ poniżej $1$/$-1$ powodują prędkości odpowiednio dodatnie/ujemne - czyli
system oddala się od punktu równowagi.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\linewidth]{fig/poincare_bendixon.PNG}
...
$$
    \frac{\partial f_{1}}{\partial x_{1}} + \frac{\partial f_{2}}{\partial x_{2}} = 2 - 4(x_{1}^{2} + x_{2}^{2})
$$
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{fig/bendixon.png}
...
\newpage
    ...
    Jednak w literaturze można odnaleźć szereg metod, które sugerują pewne obszary i metodologie poszukiwań.
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.25\linewidth]{fig/energia_lapunow.PNG}
        ...
        \begin{itemize}
            ...
            \item $F_{s} = g(x)$.
        \end{itemize}
        Gdzie $f(x,\dot{x})$ i $g(x)$ są funkcjami ciągłymi oraz spełniony jest warunek:
        \begin{itemize}
            \item $g(0) = 0$,
        \end{itemize}
        ...
        \begin{equation}
            \label{eq:mzg_1}
            \frac{\partial \nabla V_{i}}{\partial x_{j}} = \frac{\partial \nabla V_{j}}{\partial x_{i}}, \qquad \mathrm{dla} \qquad i,j=1, 2, ..., n
        \end{equation}
        ...
        \begin{enumerate}
            ...
            $$
                V(x) = \int_{0}^{x_{1}}{\nabla V_{1}(x_{1}, 0,\cdots, 0)dx_1} + \int_{0}^{x_{2}}{\nabla V_{2}(x_{1}, x_{2}, 0 \cdots, 0)dx_2} + \cdots + \int_{0}^{x_{n}}{\nabla V_{n}(x_{1}, \cdots, x_{n})dx_n}.
            $$
        ...
        czyli:
        $$
            x_{1}\frac{\partial a_{11}}{\partial x_{2}} + x_{2}\frac{\partial a_{12}}{\partial x_{2}} +a_{12} = a_{21} + x_{1}\frac{\partial a_{21}}{\partial x_{1}} +x_{2}\frac{\partial a_{22}}{\partial x_{1}}.
        $$
        
        \textbf{Wybór A}

        Współczynniki $a_{ij}$ można dobrać jako:
        $$
        ...
        $$
            \nabla V_{1} = x_{1},\,\nabla V_{2} = x_{2}.
        $$
        Funkcja Lapunowa ma postać:
        $$
            V(x) = \frac{1}{2}(x_{1}^{2} + x_{2}^{2}).
        $$
        a pochodna systemowa jest lokalnie ujemnie określona w obszarze $(1-x_1x_2)$, co wnioskujemy z jej równania:
        $$
            \dot{V}(x) = -2x_{1}^{2} - 2x_{2}^{2}(1-x_{1}x_{2}).
        $$
        \textbf{Wybór B}

        Alternatywnie współczynniki $a_{ij}$ można dobrać następująco:
        $$
            a_{11} = 1, \quad a_{12} = x_2^2, \quad a_{21} = 3x_2^2, \quad a_{22}=3
        $$
        W tym wypadku:
         $$
            \begin{array}{l}
                 \nabla V_{1} = x_{1} + x_2^3\\
                 \nabla V_{2} = 3x_{1}x_2^2 + 3x_{2}
            \end{array}
        $$
        Funkcja Lapunowa ma postać:
        $$
                V(x) = \int_{0}^{x_{1}}{(x_{1} + x_2^3)dx_1} + \int_{0}^{x_{2}}{(3x_{1}x_2^2 + 3x_{2})dx_2} = \frac{x_1^2}{2}+x_1x_2^3+\frac{3}{2}x_2^2
        $$
        Pochodna systemowa dla tego wyboru współczynników wynosi:
        $$
            \dot{V}(x) = -2x_1^2 - 6x_2^2 -2x_2^2(x_1x_2+3x_1^2x_2^2)
        $$      
        $\dot{V}(x)$ jest funkcją lokalnie ujemnie określoną.
...
\newpage
    ...
    \subsection{Przykłady}
        ...
        \begin{enumerate}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_3_v.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_3_obszar.png}
        ...
        Rozpatrzmy raz jeszcze układ z \textbf{Przykładu 1} opracowany z wykorzystaniem ukazji pośredniej metody Lapunowa:
        ...
        Spróbujmy teraz oszacować jego obszar atrakcji za pomocą narzędzi z bezpośredniej metody Lapunowa.
        ...
        \begin{enumerate}
            ...
            $$
                ...
                \end{bmatrix}.
            $$
            Przypomnijmy równanie Lapunowa:
            $$
                A^{T}P + PA = -Q,
            $$
            gdzie poszukiwanym rozwiązaniem jest macierz $P = \begin{bmatrix} p_{11} & p_{12} \\ p_{12} & p_{22} \end{bmatrix}$.
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_4_v_1.png}
                \caption{Poziomice funkcji Lapunowa na tle portretu fazowago z przykładu 4.}
                \label{fig:przyklad_4_obszar}
            \end{figure}
            Funkcja Lapunowa to $V(x) = x^T P x$, a jej pochodna systemowa ma postać:
            $$
                \dot{V}(x)
                =
                \frac{1}{2}q
                ...
                x.
            $$
            Można zauważyć, że jej znak nie zależy od wartości parametru $q$ (z założenia $q>0$) oraz dla $x_{1} \in (12.36; 0.66)$.
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_4_vp_1.png}
                \caption{Poziomice pochodnej systemowej funkcji Lapunowa na tle portretu fazowago z przykładu 4.}
                \label{fig:przyklad_4_vp}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_4_obszar_1.png}
                \caption{Estymata obszaru atrakcji na tle portretu fazowago z przykładu 4.}
                \label{fig:przyklad_4_obsza_obszar}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_4_obszar_2.png}
        ...
        \begin{equation}\label{eq:example_5_sys}
            \left\{
            \begin{array}{l}
        ...
        Obydwie będą uzyskane na podstawie przybliżenia liniowego układu i równania Lapunowa, lecz z innym doborem macierzy $Q$.
        Zatem funkcje Lapunowa będą mieć postać:
        $$
            V(x) = x^{T}Px.
        $$

        Przed rozpoczęciem poszukiwania funkcji Lapunowa wyznaczymy przybliżenie liniowe systemu (\ref{eq:example_5_sys}):
        $$
        ...
        \begin{enumerate}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_5_v_1.png}
                \caption{Portret fazowy układu (\ref{eq:example_5_sys}) z naniesionymi poziomicami funkcji Lapunowa.}
                \label{fig:przyklad_5_v_1}
            \end{figure}

            Dla uproszczenia zapisu można układ (\ref{eq:example_5_sys})  w postaci pseudoliniowej (\ref{eq:przyklad_5_form_pseudo}):
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_5_vp_1.png}
                \caption{Portret fazowy okładu (\ref{eq:example_5_sys}) z naniesionymi poziomicami pochodnej systemowej funkcji Lapunowa.}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_5_obszar_1.png}
                \caption{Portret fazowy układu (\ref{eq:example_5_sys}) z obszarem atrakcji, wynikającym z funkcji Lapunowa.}
            ...
            $$
            Można więc zauważyć, że jej poziomice będą tworzyły elipsy, gdzie dla $a>1$ dłuższa będzie półoś wzdłuż osi $OY$, natomiast dla $a<1$ dłuższa będzie półoś wzdłuż osi $OX$.
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_5_obszar_2a.png}
                \caption{Portret fazowy układu (\ref{eq:example_5_sys}) z obszarem atrakcji wynikającym z funkcji Lapunowa}
            ...
            \begin{itemize}
                \item \textit{czerwone} - z warunku na ujemny znak pochodnej - równanie (\ref{eq:przyklad_5_negative_vp}) - są to obszary niestabilne,
                \item \textit{zielone} - pochodna systemowa funkcji $V(x)$ jest tutaj ujemna, można więc sprawdzić jakie zachowanie wynika z równania dynamiki układu. A mianowicie widać, że $\dot{x}_{2} = -x_{2}$, zatem wszystkie trajektorie się tam zaczynające schodzą w kierunku osi $OX$, zatem wpadają ostatecznie przez obszar ograniczony liniami czerwonymi. Wnioskujemy więc, że obszary te należą do obszaru przyciągania punktu równowagi.
            \end{itemize}   
        \end{enumerate}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.75\linewidth]{fig/example_system.PNG}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_6.png}
        ...
        %\begin{enumerate}
            ...
            Funkcja Lapunowa uzyskana na podstawie interpretacji energetycznej:
            \begin{equation}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_6_v_1.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_6_vp_1.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.7\linewidth]{fig/przyklad_6_obszar_1.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_7.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_7_v_1.png}
        ...
        \begin{equation}
            ...
            \dot{V}(x) = -x_{2}^{2}.
        \end{equation}
        Jej wykres naniesiony na portret fazowy został przedstawiony na rysunku (\ref{fig:przyklad_7_v_2}).
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_7_vp_1.png}
        ...
        $$
        Obszar ten został przedstawiony na rysunku (\ref{fig:przyklad_7_v_2}).
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_7_obszar_1.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_8_pf.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_8_v.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{fig/przyklad_8_vp.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{fig/przyklad_9_obszara_1_b_1.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{fig/przyklad_9_obszara_1_b_2.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{fig/przyklad_9_obszara_2_b_1.png}
    ...
    %-------------------------------------------------------
    \subsubsection{Przykład 10 (Adaptacyjny tempomat samochodowy)}
    Adaptacyjny tempomat samochodowy (ACC - Adaptive Cruise Control) jest
    systemem, który automatycznie dostosowuje prędkość pojazdu do warunków
    ruchu, utrzymując bezpieczną odległość od pojazdu poprzedzającego.
    Wykorzystuje on różne czujniki, takie jak radar czy kamera, do
    monitorowania otoczenia i podejmowania decyzji dotyczących przyspieszania
    lub hamowania.
    Schematycznie jest to przedstawione na rysunku (\ref{fig:acc}).
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.25\linewidth]{fig/snipping_ACC_Case.png}
        \caption{Schemat zachowanai adaptacyjnego tempomatu samochodowego.}
        \label{fig:acc}
    \end{figure}

    Można rozważyć prosty model dynamiki pojazdu, który opisuje jego prędkość
    $v$ i położenie $x$:
    \begin{equation}
        \label{eq:acc_model}
        \begin{bmatrix}
    ...
    gdzie:
    \begin{itemize}
        \item $x$ - położenie pojazdu,
        \item $v$ - prędkość pojazdu,
        \item $m$ - masa pojazdu,
        \item $F_{r}(v) = f_{0} + f_{1}v + f_{2}v^{2}$ - siła oporu ruchu, która jest funkcją prędkości,
        \item $u$ - siła sterująca (np. siła hamowania lub przyspieszania).
    \end{itemize}
    Znając dokładnie parametry siły oporu ruchu, można zaprojektować sterowanie,
    które zapewni stabilność układu
    $$
        u = F_{r}(v) - K(v - v_{d}).
    $$
    Wówczas układ zamyka się w postaci:
    \begin{equation}
    \label{eq:acc_closed_loop}
        \begin{bmatrix}
            \dot{x}\\
            \dot{v}
        \end{bmatrix}
    ...
    \begin{equation}
    \label{eq:acc_v}
    V(x, v) = \frac{1}{2}(v - v_{d})^{2}.
    \end{equation}
    Jej pochodna systemowa ma postać:
    \begin{equation}
    \label{eq:acc_vp}
    \dot{V}(x, v) = -\frac{K}{m}(v - v_{d})^{2}.
    \end{equation}
    Jest ona ujemna wszędzie poza osią $v = v_{d}$, gdzie jest równa 0.
    Zatem na podstawie twierdzenia LaSalle'a sprawdzamy czy istnieją 
    zbiory inwariantne na tej osi:
    $$
        \begin{bmatrix}
            \dot{x}\\
            \dot{v}
        ...
        \end{bmatrix},
    $$
    widać zatem, że jeśli prędkość pojazdu osiągnie wartość zadaną, to
    pozostanie na niej, a~położenie będzie się zmieniać liniowo w~czasie.
    Zatem jest to zbiór inwariantny (zbiór $E$) i na podstawie twierdzenia LaSalle'a wszystkie trajektorie dążą do tego zbioru, co oznacza, że prędkość pojazdu będzie dążyć do wartości zadanej $v_{d}$.
    Portret fazowy z zaznaczonymi zbiorem inwariantnym pokazano na rysunku (\ref{fig:acc_pf}).
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{fig/acc_pp.eps}
        \caption{Portret fazowy układu z przykładu 10 z zaznaczonym zbiorem inwariantnym (czerwona linia).}
        \label{fig:acc_pf}
    \end{figure}
    
    %-------------------------------------------------------
    \subsubsection{Przykład 11 (Model SI rozprzestrzeniania się epidemii)}
    Model SI (Susceptible-Infected) jest jednym z najprostszych modeli
    epidemiologicznych, który opisuje rozprzestrzenianie się choroby zakaźnej
    (nie śmiertelnej, zatem populacja jest stała) w~populacji. W~modelu tym populacja jest podzielona na dwie grupy:
    osoby podatne ($S$) i osoby zakażone ($I$). Model ten zakłada, że osoby zakażone
    mogą zarażać osoby podatne, oraz pewne szanse ozdrowienia (ale nie nabierania
    odporności).
    Równania różniczkowe opisujące model SI są następujące:
    \begin{equation}
        \label{eq:si_model}
        \left\{
        \begin{array}{l}
             \dot{S} = -\beta \frac{SI}{N} + \gamma I\\
    ...
    gdzie:
    \begin{itemize}
        \item $S$ - liczba osób podatnych,
        \item $I$ - liczba osób zakażonych,
        \item $\beta$ - współczynnik zarażania, który określa, jak szybko choroba rozprzestrzenia się w populacji,
        \item $\gamma$ - współczynnik ozdrowień, który określa, jak szybko osoby zakażone zdrowieją.
    \end{itemize}
    Model ten ma nieskończenie wiele punktów równowagi, położonych na dwóch
    prostych:
    \begin{itemize}
        \item $I=0$ i $S \in \mathbb{R}$ - oznacza to, że nie ma zakażonych
        więch choroba nie może się rozprzestrzeniać,
        \item $S = \frac{\gamma}{\beta}N$ i $I \in \mathbb{R}$ - oznacza to, że
        liczba osób podatnych jest równa stosunkowi współczynnika ozdrowień do
        współczynnika zarażania, co oznacza, że choroba jest w stanie utrzymać
        się w~populacji na stałym poziomie.
    \end{itemize}
    Ponieważ choroba nie jest śmiertelna, to populacja jest stała $S+I=N$,
    zatem można zadanie uprościć do zagadnienia jednowymiarowego,
    rozważając tylko jedną z tych zmiennych, np. $I$:
    \begin{equation}
    \label{eq:si_model_reduced}
        \dot{I} = \beta I(N - I) - \gamma I = I(\beta (N - I) - \gamma),
    \end{equation}
    wówczas mamy dwa punkty równowagi: $I_{e,1} = 0$ oraz $I_{e,2} =
    \frac{\beta-\gamma}{\beta}N$.
    
    Stabilność każdego z tych punktów równowagi można określić na podstawie 
    pośredniej metody Lapunowa. Dokonując takiej analizy stwierdzamy, że
    punkt równowagi $I_{e,1} = 0$ jest niestabilny, a~punkt równowagi
    $I_{e,2} = \frac{\beta-\gamma}{\beta}N$ jest stabilny.

    A proponując funkcję Lapunowa w postaci:
    \begin{equation}
    \label{eq:si_v}
    V(I) = \frac{1}{2}(I - I_{e,2})^{2},
    \end{equation}
    jej pochodna systemowa ma postać:
    \begin{equation}
        \label{eq:si_vp}
        \dot{V}(I) = -\frac{I(\gamma - \beta + I\beta)^2}{\beta}
    \end{equation}
    i zeruje się jedynie w punktach równowagi, zatem na podstawie twierdzenia
    LaSalle'a wszystkie trajektorie (poza punktem równowagi $I_{e,1} = 0$) dążą do stabilnego punktu równowagi
    $I_{e,2} = \frac{\beta-\gamma}{\beta}N$.
    Funkcję Lapunowa i jej pochodną systemową pokazano na rysunku (\ref{fig:si_v_vp}).
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{fig/si_v_vp.eps}
        \caption{Funkcja Lapunowa (\ref{eq:si_v}) i jej pochodna systemowa (\ref{eq:si_vp}) dla modelu SI.}
        \label{fig:si_v_vp}
    \end{figure}

    Portret fazowy układu oryginalnego z zaznaczonymi punktami równowagi
    pokazano na rysunku (\ref{fig:si_pf}).
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{fig/si_pf.eps}
        \caption{Portret fazowy modelu SI z zaznaczonymi punktami równowagi.}
        \label{fig:si_pf}
    \end{figure}
    Można zauważyć, że w przeciwieństwie do przykładu poprzedniego, zaznaczone
    odcinki nie są zbiorami przyciągania/odpychania. Są natomiast zbiorem punktów równowagi -
    jeśli trajektoria wyląduje na zielonym odcinku to nie będzie się po nim
    poruszać, ale pozostanie w punkcie, w którym się znalazła.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\section{Jak wykorzystać to do projektowania sterowania?}
    ...
    Można zatem zauważyć, że sterowanie stabilizujące punkt równowagi zostanie osiągnięte, gdy zajdzie warunek: dla każdego $x \in \{ x \in \mathbb{R}^{n}: g(x) = 0 \land x \neq 0 \}$ musi zachodzić $\mathcal{L}_{f}V(x) < 0$.
    ...
    $$
    \begin{itemize}
        \item Zakładamy, że można sterować układem bezpośrednio za pomocą zmiennej $x_{2}$ - rozważamy tylko pierwsze równanie i stosujemy do niego funkcję Lapunowa:
        $$
