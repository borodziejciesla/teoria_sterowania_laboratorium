...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{fig/dyskretny_schemat.png}
...
$$
    x^{D}(t) = \sum_{k=-\infty}^{\infty}{x(Tk)\cdot\delta(t-Tk)}.
$$
    ...
    $$
        \dot{x}(t) = f(x(t),u(t)).
    $$
    Jednak ponieważ będą nas interesowały jedynie wartości funkcji w~dyskretnych chwilach czasowych, możliwe do wyznaczenia na podstawie poprzednich, to możemy posłużyć się ogólną formułą:
    \begin{equation}
    ...
    \end{equation}
        ...
        \begin{enumerate}[(a)]
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{fig/backward_int.png}
                \caption{Aproksymacja lewostronna (\textit{backward}).}
                \label{fig:backward_int}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{fig/forward_int.png}
                \caption{Aproksymacja prawostronna (\textit{forward}).}
                \label{fig:enter-label_right}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{fig/tustin_int.png}
                \caption{Aproksymacja symetryczna.}
                \label{fig:enter-label_symmetric}
            \end{figure}
        \end{enumerate}
        ...
        $$
        Zakładając, że chwila początkowa $t_{0}$ będzie początkiem $(k+1)$-tego
        okresu próbkowania $t_{0} = Tk$, można wyznaczyć dokładna wartość na
        jego końcu - $x[T(k+1)]$:
        $$
        ...
        $$
            \tau=Tk \rightarrow \xi = T,\,\tau =Tk + s \rightarrow \xi = T-s
        $$
        ...
        \begin{enumerate} [(a)]
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{fig/forward_transform.png}
                \caption{Przekształcenie lewej półpłaszczyzny dla aproksymacji \textit{forward}.}
                \label{fig:forward_transform}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{fig/backward_transform.png}
                \caption{Przekształcenie lewej półpłaszczyzny dla aproksymacji \textit{backward}.}
                \label{fig:backward_transform}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{fig/tustin_transform.png}
                \caption{Przekształcenie lewej półpłaszczyzny dla aproksymacji Tustina.}
                \label{fig:tustin_transform}
            \end{figure}
        \end{enumerate}

        \begin{tcolorbox}[colback=green!5, colframe=green!75!black, title=Przykład]
        Rozważmy transmitancję obiektu inercyjnego pierwszego rzędu:
        $$
            G(s) = \frac{a}{s+a}
        $$
        \begin{itemize}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.5\linewidth]{fig/implse_to_zoh.png}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.5\linewidth]{fig/zoh.png}
    ...
    \begin{enumerate}[(a)]
        ...
        $$
            \frac{x_{k+1}-x_{k}}{T} = -ax_{k} \implies x_{k+1} = (1-aT)x_{k} = f[x_{k}], \, T>0.
        $$
    ...
    W przypadku układów sterowania, gdzie nie mamy z góry zadanego sygnału wyjściowego, zatem i jego częstotliwości, to graniczna wartość częstotliwości może być wyznaczona jako częstotliwość odcięcia (czyli częstotliwość, dla której tłumienie staje się większe od 3 dB), odczytana z charakterystyki Bodego obiektu sterowania.
        ...
        %--------------------------------------------------
        \subsubsection{Reguła krotności pasma}
        W praktyce inżynierskiej częstotliwość próbkowania
        $f_{p}$ dobiera się jako wielokrotność częstotliwości granicznej
        obiektu $f_g$ (częstotliwości odcięcia). Powszechnie stosowane
        przedziały to:
        \begin{itemize}
            \item dla układów o niskich wymaganiach: $f_p \approx 10 \cdot f_g$,
            \item dla układów o wysokich wymaganiach: $f_p \approx (20\div40) \cdot f_g$.
        \end{itemize}

        %--------------------------------------------------
        \subsubsection{Próbkowanie a czas odpowiedzi skokowej}
        Jeśli dysponujemy charakterystyką czasową obiektu, okres
        próbkowania $T_p$ można odnieść do parametrów czasowych:
        \begin{itemize}
            \item czas narastania ($\tau_r$): zaleca się, aby
            w~czasie narastania sygnału mieściło się od 4 do 10 próbek.
            \item stała czasowa ($\tau$): Dla obiektów inercyjnych często przyjmuje się $T_p \approx 0.1 \cdot \tau$.
        \end{itemize}

        %--------------------------------------------------
        \subsubsection{Ograniczenia sprzętowe i "Real-Time"}
        Nie bez znaczenia w przypadku sterowania cyfrowego jest
        też minimalny czas konieczny na realizację sterowania.
        Czyli czas wykonywania pomiaru, przesłania z czujnika do
        układu mikroprocesorowego, czas wyznaczenia sterowania oraz
        czas konieczny na przesłanie sygnału sterującego do elementu
        wykonawczego.
        W systemach czasu rzeczywistego mus być zatem spełniony warunek:
        $$
            T_p > T_{pomiar} + T_{przeslanie} + T_{obliczenie} + T_{przeslanie\_sterowania},
        $$
        \begin{itemize}
            \item $T_{pomiar}$ - czas wykonywania pomiaru,
            \item $T_{przeslanie}$ - czas przesyłania danych z czujnika do układu sterującego,
            \item $T_{obliczenie}$ - czas obliczania sterowania,
            \item $T_{przeslanie\_sterowania}$ - czas przesyłania sygnału sterującego do elementu wykonawczego.
        \end{itemize}

        %--------------------------------------------------
        \subsubsection{Zjawisko aliasingu i filtry antyaliasingowe}
        Należy pamiętać, że jeśli w sygnale pomiarowym występują zakłócenia 
        o~częstotliwościach wyższych niż częstotliwość Nyquista ($f_N = f_p/2$),
        zostaną one „nałożone” na pasmo użyteczne jako błędy
        niskoczęstotliwościowe.
        Aby temu zapobiec, przed przetwornikiem ADC stosuje się analogowe
        filtry dolnoprzepustowe (antyaliasingowe).


    \begin{tcolorbox}[colback=orange!5, colframe=orange!75!black, title=Uwaga]
        Zbyt wysoka częstotliwość próbkowania również może być
        problematyczna - prowadzi do dużych błędów przy obliczaniu
        różniczek (człon D w regulatorze PID) ze względu na szum kwantyzacji.
    \end{tcolorbox}
...
\section{Przykłady}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{fig/przyklad_1.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{fig/Screenshot from 2024-05-12 15-53-29.png}
        \caption{Model symulacyjny dla przykładu 2.}
        \label{fig:sim_example_2l}
    \end{figure}

    Wyniki symulacji przedstawiono na rysunku(\ref{fig:wyjscie_przyklad_2}).
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{fig/przyklad_2_dyskretne.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{fig/przyklad_2_dyskretne_2.png}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{fig/przyklad_3_dyskretne.png}
        \caption{Przykładowe przebiegi dla układu z przykładu 3.}
        \label{fig:przyklad_3_dyskretne}
    \end{figure}
...
\section{Jak to zrobić w MATLABie?}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{fig/simulink_model.png}
