...
\section{Wprowadzenie}
    ...
    Aby URA był \textbf{wewnętrznie} stabilny, każda z tych transmitancji z Tabeli \ref{tab:uklad_regulacji_transmitancje} musi być co najmniej stabilna.
    ...
    W zależności od doboru wskaźnika jakości, parametry układu będą dobierane w inny sposób.
        ...
        \begin{enumerate}[(a)]
            ...
            \textit{Integral Square Output} - \textbf{ISO}) (dla stabilizacji w zerze):
                \begin{equation}
                    \label{eq:j7}
                    J_{7} = \int_{0}^{\infty}{y^{2}(t)dt},\,y(t) \in \mathbb{R}
                \end{equation}
    
            \item wskaźnik mieszany uwzględniający koszt sterowania:
                \begin{equation}
                    \label{eq:j8}
                    J_{8} = \int_{0}^{\infty}{(\alpha_{1}e(t)^{2} + \alpha_{2}u(t)^{2})dt}, \alpha_{1}, \alpha_{2} \in \mathbb{R}_{+}.
                \end{equation}
        ...
        co jest uzasadnione teoretycznie, ponieważ dla stabilnych układów regulacji
        sygnały $e(t)$, $u(t)$ i $y(t)$ dążą do zera, a~więc całki te są
        skończone.
        W~dodatku jest to też dogodne dla obliczeń analitycznych.
...
\section{Regulator liniowo-kwadratowy}
    ...
    że optymalna sterowanie w~danym momencie czasu
    ...
    tej trajektorie odpowiadające przedziałowi czasu $[t_1, t_f]$
    ...
    W przypadku poszukiwania optymalnej sterowania dla systemu dynamicznego:
    \[
        \dot{x}(t) = f(x(t), u(t)),
    \]
    ...
    \begin{tcolorbox}[colback=orange!5, colframe=orange!75!black, title=Uwaga]
        ...
        uczenia maszynowego - uczenia zee wzmocnieniem (ang. Reinforcememt
        Learning).
        Takich jak Q-learning, gdzie funkcja wartości jest
    ...
    \[
        \dot{x}(t) = Ax(t) + Bu(t),
    \]
    gdzie $A$ i $B$ są macierzami systemu, a~$H$ i~$H$ są
    macierzami wag dla stanu i~sterowania odpowiednio.

    Natomiast poszukujemy regulatora liniowego, proporcjonalnego do stanu, w~postaci:
    \[
        u(t) = -Kx(t),
    \]
    ...
    %======================================================
    \subsection{Zastosowanie regulatora liniowo-kwadratowego}
    ...
    sterowania, co jest często pożądane w~praktyce, aby uniknąć nadmiernego
