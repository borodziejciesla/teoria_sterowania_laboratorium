    ...
    W zależności od doboru wskaźnika jakości, parametry układu będą dobierane w inny sposób.
        ...
        \begin{enumerate}[(a)]
            ...
            \item wskaźnik mieszany wykorzystywany w sterowniku LQR:
                \begin{equation}
                    \label{eq:j8}
                    J_{LQR} = \int_{0}^{\infty}{(x^{T}(t)Qx(t) + u^{T}(t)Rx(t))dt}, \qquad Q=Q^{T}>0, \qquad R=R^{T}>0.
                \end{equation}
        \end{enumerate}
...
\section{Regulator liniowo-kwadratowy}
    ...
    \[
        J = \int_{0}^{T}{\left( x(t)^{T}Qx(t) + u(t)^{T}Ru(t) \right)dt} \implies L(x,u) = x(t)^{T}Qx(t) + u(t)^{T}Ru(t),
    \]
    przy czym układ jest opisywany liniowym równaniem stanu:
    \[
        \dot{x}(t) = Ax(t) + Bu(t),
    \]
    gdzie $A$ i $B$ są macierzami systemu, a~$Q$ i~$R$ są
    ...
    otrzymujemy:
    \[
        0 = 2Ru + 2PB^{T}x \implies u = -R^{-1}B^{T}Px.
    \]
    ...
    \[
        0 = x(t)^{T}Qx(t) + x(t)^{T}P\left( A - BR^{-1}B^{T}P \right)x(t),
    \]
