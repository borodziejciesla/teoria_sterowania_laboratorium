...
$$
    x(t+T) = x(t)
$$
dla pewnego $T>0$ i każdego $t>t_{0}$, to nazywamy je \textbf{rozwiązaniem
okresowym}.
Jeśli ponadto jest to trajektoria cykliczna izolowana, to nazywamy ją \textbf{cyklem granicznym}. Trajektoria w przestrzeni stanów, odpowiadająca cyklowi granicznemu, ma postać zamkniętej krzywej.
...
skończonej liczby - może być ich dowolna liczba, nawet nieskończenie wiele.
...
Jeżeli pochodna systemowa $\dot{V}:D \rightarrow \mathbb{R}$ jest ujemnie określona w $D$, to wzdłuż trajektorii $x(t) = x(t; t_{0}, x_{0})$ układu (\ref{eq:sys_stac}) funkcja $V(x)$ jest ściśle malejąca i ograniczona od dołu przez 0, więc zbiega do nieujemnej granicy $c$:
$$
