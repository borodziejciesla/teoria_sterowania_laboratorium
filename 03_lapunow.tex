...
\end{figure}
Niekoniecznie musi być tylko jeden cykl graniczny, może być ich dowolna ilość, nawet nieskończenie wiele, jak w układzie (\ref{eq:cykle_graniczne}):
...
\begin{equation}\label{eq:sys_lin}
    \dot{x} = Ax
\end{equation}
dlatego można wykazać, że jeśli istnieje dodatnio określona i symetryczna macierz $P \in \mathbb{R}^{n}$, która spełnia tzw. równanie Lapunowa:
...
\textbf{Intuicja}: Z twierdzenia tego wynika, iż zbiór poziomicowy funkcji Lapunowa jest zbiorem niezmienniczym.
...
\end{figure}
    ...
    Jednak w literaturze można odnaleźć szereg metod, które sugerują pewne obszary i metodologie poszukiwań.
        ...
        \begin{itemize}
            ...
            \item $E_{p} = \int_{0}^{x_{1}}{g(z)dz}$ - eneria potencjalna.
        \end{itemize}
...
\section{Jak wykorzystać to do projektowania sterowania?}
    ...
    \begin{description}
        ...
        \textbf{Przykład}: stabilizujemy układ w punkcie $x_{e} = 0$:
        $$
        ...
        $$
            ...
            \end{array}.
        $$
        Dość łatwo zauważyć, że metoda ts jest bardzo elastyczna, można wprowadzić inny wskaźnik jakości oraz inne ograniczenia.
    \end{description}
    ...
    \textbf{Przykład}: celem jest stabilizacja zerowego punktu równowagi układu:
    $$
        \left\{
        \begin{array}{l}
