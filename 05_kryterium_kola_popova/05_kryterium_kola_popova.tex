...
\section{Wprowadzenie}
    ...
    \subsection{Definicje}
        ...
        Transmitancja $G(s)$ przedstawia tzw. system (ściśle) dodatnio rzeczywisty (\textit{ang.} \textit{\textbf{(S)PR} - (Strictly) Positive Real}), jeśli jej część rzeczywista $Re\left\{P(\omega)\right\}$ jest zawsze większa lub równa (większa) zeru: $Re(G(s)) \geq (>) 0$.

        \textbf{Twierdzenie}: Transmitancja $G(s)$ jest ściśle dodatnio rzeczywista wtedy i tylko wtedy, gdy:
        \begin{itemize}
    ...
    \begin{tcolorbox}[colback=pink!5, colframe=pink!75!black, title=Intuicja]
        ...
        zamkniętego.
        Wyobraźmy sobie teraz eweolcję tego kryterium, gdy wprowadzamy
        niepewność warości regulatora, czyli gdy $K$ może przyjmować wartości
        ...
        \begin{itemize}
            ...
            chwili, nie możemy pozwolić, odcinek ten "puchnie" do koła,
            aby uwzględnić wszystkie możliwe kombinacje i zapewnić stabilność.
        \end{itemize}
    ...
    \begin{tcolorbox}[colback=orange!5, colframe=orange!75!black, title=Uwaga]
        Kryterium koła jest bardziej ogólne niż kryterium Popova, ponieważ pozwala na niestacjonarność funkcji nieliniowej $\varphi(y,t)$ oraz rozszerza zakres funkcji nieliniowych do sektora $[k_{1}, k_{2}]$.
         Jednakże, kryterium koła jest również bardziej restrykcyjne w porównaniu do kryterium Popova, ponieważ wymaga spełnienia dodatkowych warunków dotyczących macierzy stanu $A$ oraz istnienia $k_{0}$, dla którego $Re\left\{ \lambda(A-bk_{0}c^{T}) \right\} < 0$.
    \end{tcolorbox}
